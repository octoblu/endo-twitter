...
class PostDirectMessagesNew
  ...
  do: ({data}, callback) =>
    data ?= {}
    return callback @_userError(422, 'Text is required') unless data.text?

    @twitter.post 'direct_messages/new', data, (error, data) =>
...
module.exports = PostDirectMessagesNew
